  ...
  return (
    <div className="w-full top-0 z-60 h-0">
      <div className="px-4 flex justify-center py-5">
        <header className={`flex w-full max-w-[1536px] min-h-14 items-center justify-between whitespace-nowrap px-4 md:px-10 py-3 ${showLogo ? 'border-b border-solid md:border-none border-secondary/10' : ''}`}>
          ...
          <div id="mobile-menu" className={`fixed inset-0 bg-cream z-50 transform transition-transform duration-300 ease-in-out md:hidden ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            <div className="flex flex-col items-center justify-start gap-8 h-full">
              <a href="/" className={`font-rye text-3xl text-dark-brown ${activePage === 'home' ? 'text-primary' : ''}`}>Home</a>
              <a href="/nfc" className={`font-rye text-3xl text-dark-brown ${activePage === 'nfc' ? 'text-primary' : ''}`}>NFC</a>
              <a href="/events" className={`font-rye text-3xl text-dark-brown ${activePage === 'events' ? 'text-primary' : ''}`}>Events</a>
              <a href="/about" className={`font-rye text-3xl text-dark-brown ${activePage === 'about' ? 'text-primary' : ''}`}>About</a>
            </div>
          </div>
        </header>
      </div>
